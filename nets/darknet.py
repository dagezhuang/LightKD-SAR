...
class CSPLayer(nn.Module):
    def __init__(self, in_channels, out_channels, n=1, shortcut=True, expansion=0.5, depthwise=False, act="silu",):
        ...
        # 使用CustomInvertedResidual替换原来的Bottleneck
        module_list = [CustomInvertedResidual(
...
# filePath: yolox-pytorch-main/nets/darknet.py（修改后）
class CSPDarknet(nn.Module):
    def __init__(self, dep_mul, wid_mul, out_features=("dark3", "dark4", "dark5"), depthwise=False, act="silu",):
        ...
        base_channels = int(wid_mul * 64)  # 动态计算基础通道
        base_depth = max(round(dep_mul * 3), 1)
        
        self.stem = Focus(3, base_channels, ksize=3, act=act)

        # 每层输出通道动态计算（关键：支持学生/教师模型不同width）
        ...
        # 新增：记录各层输出通道（供蒸馏时特征对齐使用）
        self.out_channels = {
            "dark2": base_channels * 2,
            "dark3": base_channels * 4,
            "dark4": base_channels * 8,
            "dark5": base_channels * 16,
        }
